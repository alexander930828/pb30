...
def  test(n):
    ...
    # 느린 O(n*n) 알고리즘 테스트
    start = time.time() # 계산 시간 직전 시간을 기억
    mps = max_profit_slow(a) # 계산 수행
    end = time.time()# 계산 시간 직후 시각을 기억
    time_slow = end - start # 직후 시각에서 직전 시간을 뺴면 계산에 걸린 시간
    # 빠른 O(n) 알고리즘 테스트
    start = time.time() # 계산 시간 직전 시간을 기억
    mps = max_profit_slow(a) # 계산 수행
    end = time.time()# 계산 시간 직후 시각을 기억
    ...
    if time_fast > 0: # 컴퓨터 환경에 따라 빠른 알고리즘 시간이 0으로 측정될 수 있음 # 이럴때는 0을 출력
        m = time_slow / time_fast # 느린 알고리즘 시간 / 빠른 알고리즘 시간
    # 입력크기, 느린 알고리즘 수행 시간, 빠른 알고리즘 수행 시간, 계산 차이
    # %d는 정수 출력, %.5f는 소수점 다섯 자리까지 출력을 의미
    print("%d %.5f %.5f %.2f" % (n, time_slow, time_fast, m))

test(100)
test(10000)
# test(10000) # 수행 시간이 오래 걸리므로 일단 주석 처리리